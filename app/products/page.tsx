...
export default function AllProductsPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Remove duplicate products by using a Map with trimmed product name as key
        const uniqueProducts = new Map();
        allProducts.forEach(product => {
          if (!uniqueProducts.has(product.name.trim())) {
            uniqueProducts.set(product.name.trim(), product);
          }
        });
  ...
  return (
    <div className="container mx-auto px-4 py-12 md:px-6 lg:py-16">
      <CategoryHeader
        title="All Products"
        description="Browse our complete collection of abayas"
      />
      ...
      <div className="mt-8 flex flex-col gap-6 lg:flex-row">
        ...
        <div className="flex-1">
          ...
          {loading ? (
            ...
            console.log("Filtered Products for rendering:", filteredProducts),
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4">
              {filteredProducts.map((product) => {
                // Check if product has hijab using the has_hijab field
                const hasHijab = product?.has_hijab || false;

                return (
                  <ProductCard 
                    key={product.productId} 
                    product={product}
                    isInWishlist={wishlistItems.some((item) => item.product_id === product.productId)}
                    onAddToWishlist={() => handleAddToWishlist(product)}
                    saleLabel={product.is_new_arrival ? "Save 5%" : "Exclusive Discount"}
                    showHijabLabel={hasHijab}
                  />
                );
              })}
            </div>
